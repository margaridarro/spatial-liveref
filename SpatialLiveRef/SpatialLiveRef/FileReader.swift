...
func projectParametersAnalysis(files: [String:FileBuildingEntity]) -> (locMultiplier: Float, nomMultiplier: Float){
...
/*
func orderFileByPath(files: [String:FileBuildingEntity]) -> [String] {
    
    var path = files[files.keys.startIndex].key.split(separator: "/")
    let fileTree = [String(path[0])]
